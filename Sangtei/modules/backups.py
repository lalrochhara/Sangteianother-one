...
from Sangtei import dispatcher, log as LOGGER, OWNER_ID
from Sangtei.__main__ import DATA_IMPORT
from Sangtei.modules.helper_funcs.alternate import typing_action
from Sangtei.modules.helper_funcs.decorators import kigcmd
...
def export_data(update, context):  # sourcery no-metrics
    ...
    with open("Sangtei{}.json".format(chat_id), "w") as f:
        f.write(str(baccinfo))
    context.bot.sendChatAction(current_chat_id, "upload_document")
    tgl = time.strftime("%H:%M:%S - %d/%m/%Y", time.localtime(time.time()))
    context.bot.sendDocument(
        current_chat_id,
        document=open("Sangtei{}.json".format(chat_id), "rb"),
    ...
    os.remove("Sangtei{}.json".format(chat_id))  # Cleaning file
